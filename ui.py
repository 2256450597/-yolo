...
class mainwindow(QMainWindow, Ui_MainWindow):
    ...
    def model_init(self):
        ...
        self.comboBox_model.addItem('自训练模型sit')
        self.comboBox_model.addItem('自训练模型瓦')
        # 信号与槽绑定模型选择函数
    ...
    def get_image_path(self):
        ...
        if path[0]:
            ...
            # 将检测结果提取，作为参数进行格式化打印
            self.detection_num_print(result[0])
    ...
    # 检测结果数量统计打印函数
    # 功能模块：检测结果数量统计逻辑
    def detection_num_print(self, det_result):
        """
        统计并打印每种检测目标的数量
        ...
        # 获取检测结果中的类别索引矩阵
        det_cls = det_result.boxes.cls
        # 将cls矩阵转化为能够正常提取成员的numpy矩阵
        det_cls_arr = det_cls.numpy()
        
        # 统计数量
        count_dict = {}
        for cls_idx in det_cls_arr:
            cls_name = det_dict[int(cls_idx)]
    ...
    def video_init(self):
        ...
        self.video_timer.setInterval(500)  # 约30fps的播放速度
        # 视频相关变量初始化
